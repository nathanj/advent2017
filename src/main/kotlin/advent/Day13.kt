...
data class Firewall(val range: Int,
                    val depth: Int,
                    var position: Int = 0,
    ...
    fun clone() = Firewall(range, depth, position, direction)
}

fun ridePacket(layers: ArrayList<Firewall?>): Pair<Boolean, Int> {
    var packetPos = -1
    var severity = 0
    var caught = false
    while (++packetPos < layers.size) {
        layers[packetPos]?.let { firewall ->
            if (firewall.position == 0) {
                severity += packetPos * firewall.range
                caught = true
            }
        }
        layers.filterNotNull().forEach(Firewall::update)
    }

    return Pair(caught, severity)
}

fun ridePacket2(layers: ArrayList<Firewall?>): Boolean {
    var packetPos = -1
    while (++packetPos < layers.size) {
        layers[packetPos]?.let { firewall ->
            if (firewall.position == 0) {
...
fun parseFirewall(lines: List<String>): ArrayList<Firewall?> {
    val layers = ArrayList<Firewall?>(100)
    ...
    for (line in lines) {
        line.split(": ").let {
            layers[it[0].toInt()] = Firewall(range=it[1].toInt(), depth=it[0].toInt())
        }
    }
    return layers
}

fun delayPacket(lines: List<String>): Int {
    var delay = 0
    val layers = parseFirewall(lines)
    while (true) {
        val clone = ArrayList(layers)
        clone.forEachIndexed { index, firewall ->
            if (firewall != null)
                clone[index] = firewall.clone()
        }
        if (!ridePacket2(clone))
            return delay
        delay++
        layers.filterNotNull().forEach(Firewall::update)
    }
...
fun day13() {
    ...
    println("Day 13 Part 1: ${ridePacket(parseFirewall(lines)).second}")
    println("Day 13 Part 2: ${delayPacket(lines)}")
}
