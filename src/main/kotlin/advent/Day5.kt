...
fun mazeSteps(offsets: IntArray, part: Int = 1): Int {
    var pos = 0
    var steps = 0
    val offs = offsets.clone()
    do {
        val o = offs[pos]
        if (part == 1)
            offs[pos]++
        else
            offs[pos] = if (o >= 3) o - 1 else o + 1
        pos += o
        steps++
    } while (pos < offs.size)
    return steps
...
fun day5() {
    val offsets = File("day5.txt").readLines().map(String::toInt).toIntArray()
    println("Day  5 Part 1: ${mazeSteps(offsets)}")
    println("Day  5 Part 2: ${mazeSteps(offsets, part = 2)}")
}
