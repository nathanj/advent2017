...
fun memCycles(banks: IntArray, part: Int = 1): Int {
    val seen = ArrayList<String>()
    val banks = banks.clone()
    var cycles = 0

    while (!seen.contains(banks.contentToString())) {
        val index = banks.maxIndex()
        val n = banks[index]
    ...
    if (part == 1)
        ...
        seen.forEachIndexed { index, s ->
            if (s == banks.contentToString())
...
fun day6() {
    val banks = input.split("\\s+".toRegex()).map(String::toInt).toIntArray()
    println("Day  6 Part 1: ${memCycles(banks)}")
    println("Day  6 Part 2: ${memCycles(banks, part = 2)}")
}
